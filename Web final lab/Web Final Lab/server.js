...
const authApiRoutes = require('./routes/api/v1/auth');
const productsApiRoutes = require('./routes/api/v1/products');
const userApiRoutes = require('./routes/api/v1/user');
const ordersApiRoutes = require('./routes/api/v1/orders');
...
// Express EJS Layouts setup (used for /onsale-products page)
app.use(expressLayouts);
app.set('layout', false); // Disabled by default — only onsale page uses layout
...
// ===== ON-SALE PRODUCTS PAGE (FINAL LAB EXAM) =====
app.get('/onsale-products', async (req, res) => {
  try {
    // Step 1: Query DB for all products where isOnSale is true
    const onSaleProducts = await Product.find({ isOnSale: true });
    
    console.log(`🏷️  Found ${onSaleProducts.length} on-sale products`);
    
    // Step 2: Pass the entire array to the EJS view using the layout
    res.render('onsale', {
      title: 'On Sale Products',
      products: onSaleProducts,
      layout: 'layouts/main'  // Enable layout ONLY for this page
    });
  } catch (err) {
...
app.use('/api/v1/products', productsApiRoutes);
app.use('/api/v1/user', userApiRoutes);
app.use('/api/v1/orders', ordersApiRoutes);
